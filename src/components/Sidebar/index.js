...
function Sidebar() {
  ...
  const init = () => {
    ...
    let getMainPathName = "";
    let getSubPathName = "";
    const mainPathName = ["products", "manageOrders", "inbox", "helpCenter"];
    const subPathName = [
      "dashboard",
      "searchProduct",
      "importList",
    ...
    if (getBasicPathName.search(mainPathName[0]) !== -1) {
      ...
      getMainPathName = 3;
    } else if (getBasicPathName.search(mainPathName[2]) !== -1) {
      getMainPathName = 4;
    } else if (getBasicPathName.search(mainPathName[3]) !== -1) {
      getMainPathName = 5;
    } else {
      getMainPathName = 0;
    }
    ...
    if (getBasicPathName.search(subPathName[0]) !== -1) {
      ...
      getSubPathName = 7;
    } else if (getBasicPathName.search(subPathName[7]) !== -1) {
      getSubPathName = 8;
    } else {
      getSubPathName = 9;
    }
      ...
      subMenus: [
        {
          subId: 7,
          subIcon: "OST",
          subTitle: "Open Support Ticket",
          to: "/helpCenter/ticket",
        },
        {
          subId: 8,
          subIcon: "F",
          ...
          {menuArray.map((menu) => {
            if (!Array.isArray(menu.subMenus)) {
              ...
              return (
                <li key={menu.id}>
                  <div
                    className={isMainMenuActive === menu.id ? "main-menu-active tab-menu" : "tab-menu"}
                    onClick={() => {
                  ...
                  <ul className={menu.collapseStatus ? "menu-show tab-flex-column" : "menu-hide"}>
                    {menu.subMenus.map((subMenu) => (
                      <li key={subMenu.subId}>
                        <Link
                          className={isActive === subMenu.subId ? "menu-active tab-menu" : "tab-menu"}
                          to={subMenu.to}
